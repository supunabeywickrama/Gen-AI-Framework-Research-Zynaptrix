...
class RAGGenerator:
    ...
    def _build_conversational_wizard_prompt(self, manual_id: str, user_query: str, text_context: str, image_references: list, history: str) -> str:
        ...
        return (
            ...
            "CURRENT CONTEXT:\n"
            f"1. CHAT HISTORY (User's progress so far):\n{history}\n"
            f"2. OPERATOR'S RECENT INPUT: '{user_query}'\n"
            f"3. MANUAL SOURCE OF TRUTH (Procedures and diagrams):\n{text_context}\n\n"
            "YOUR TASK:\n"
            "1. SAFETY PHASE FIRST: Look at the CHAT HISTORY. If there is NO message from the AGENT that starts with '[PHASE: Safety & Lockout]', you MUST start your response with '[PHASE: Safety & Lockout]'. This phase is NON-NEGOTIABLE. Prioritize Safety, Lockout/Tagout, and PPE strictly before any repair.\n"
            "   - If safety is already affirmed in history, use '[PHASE: Inspection & Repair]' for subsequent steps.\n"
            "2. ADAPT TO OPERATOR STATE:\n"
            "   - IF the input contains '[STUCK_STEP]': The operator failed the current task. DO NOT advance to the next step. Look in the manual for 'Alternative Ways' or troubleshooting branches. Write out a detailed, empathetic guide for non-technical persons to overcome this specific blocker.\n"
            "   - IF the input contains '[CLARIFY_STEP]': The operator wants to know 'How'. DO NOT advance. Break down the current task into very simple, detailed sub-steps for a beginner.\n"
            "   - IF the input contains '[EVALUATE_STEP]' or they confirm completion: They succeeded. You MUST READ the 'MANUAL SOURCE OF TRUTH', determine the exact NEXT logical step that follows, and write it out as a full instruction. DO NOT just tell the user to 'proceed to the next step' - you must be the one to provide the actual step details.\n"
            "3. MENTOR PERSONA: For standard progression, write exactly 2-3 detailed sentences. Explain WHAT to do, WHY it matters, and HOW to do it. Use warm, encouraging English.\n\n"
            "OUTPUT FORMAT:\n"
            "- Start with '[PHASE: <Name>]' as defined above.\n"
            "- Do NOT output raw tags like [STEP_COMPLETE] or [STEP_NEED_HELP] in your final text. Just write the response.\n"
            "- If the ENTIRE manual procedure is fully resolved and tested, output exactly '[PROCEDURE_FINISH]' at the very end.\n\n"
            f"AVAILABLE IMAGES:\n{image_tags}"
        )
