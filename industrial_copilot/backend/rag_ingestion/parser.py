...
class DocumentParser:
    ...
    def parse_pdf(self, file_path: str, manual_id: str):
        ...
        try:
            print(f"Extracting tables from {file_path} using Camelot...")
            tables = camelot.read_pdf(file_path, pages='all', flavor='lattice')
            for table in tables:
                df = table.df
                table_str = df.to_csv(index=False) # CSV format is often better for LLMs than JSON
                parsed_data.append({
                    "type": "table",
                    "content": table_str,
                    "page": table.page
            ...
            print(f"No tables found or Camelot extraction failed: {e}")
